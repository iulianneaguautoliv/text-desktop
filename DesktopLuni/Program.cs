...
namespace DesktopLuni
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            IFereastra f5 = new AddobeReader("Cititor",10,10,20,35);
            //f1.Minimize();
            // f2.Minimize();
            ...
            {
                (f2 as ITextEditor).AddText("CSharp Rules Break Rules", 2, 3);
                (f2 as ITextEditor).AddText("Afara este frumos", 4, 5);
            }
            if (f4 is IFileReader)
            {
                (f4 as IFileReader).ReadTextFile("C://temp/text.txt");
            }
